                ...
                <div class="col-lg-4 mt-5 cart-wrap ftco-animate">
                    ...
                    <div class="courier-options">
                        ...
                        <div class="courier-card">
                            ...
                            <a class="btn btn-primary select-courier"
...
$(document).ready(function() {
    $('.select-courier').click(function() {
        ...
        $.ajax({
            url: "<?php echo site_url('shop/get_shipping_cost'); ?>",
            type: "POST",
            data: {
                courier: courier,
            ...
            success: function(response) {
                if (response.success) {
                    // Update ongkos kirim dan total harga
                    $('#shipping_cost').text('Ongkir: Rp ' + response.shipping_cost);
                    var totalPrice = response
                        .total_price; // Misalnya, total harga sudah dikirimkan bersama dengan ongkir
                    $('.n-total').text('Rp ' + formatRupiah(
                        totalPrice)); // Update total harga

                    // Update status ongkir
                    if (response.free_shipping) {
                        $('.n-ongkir').text('Gratis');
                    } else {
                        $('.n-ongkir').text('Rp ' + formatRupiah(response.shipping_cost));
                    }
                } else {
