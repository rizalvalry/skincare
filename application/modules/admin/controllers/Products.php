...
class Products extends CI_Controller {
        ...
        else
        {
			...
			if (!empty($sub_product_names) && !empty($sub_product_prices)) {
				foreach ($sub_product_names as $key => $sub_name) {
					if (!empty($sub_name) && isset($sub_product_prices[$key]) && !empty($sub_product_prices[$key])) {
    ...
    public function edit($id = 0)
    {
        if ( $this->product->is_product_exist($id))
        {
            ...
            $product['categories'] = $this->product->get_all_categories();
			$product['sub_products'] = $this->product->get_sub_products_by_product_id($id);
	...
	{
		// Set delimiter untuk pesan error validasi
		$this->form_validation->set_error_delimiters('<div class="form-error text-danger font-weight-bold">', '</div>');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->edit($id); // Kembali ke halaman edit jika validasi gagal
		} else {
			$id = $this->input->post('id');
			$data = $this->product->product_data($id);
			...
			$product_data = [
				...
				'price' => $this->input->post('price'),
				'current_discount' => $this->input->post('price_discount'),
				'stock' => $this->input->post('stock'),
				'product_unit' => $this->input->post('unit'),
				'is_available' => $this->input->post('is_available') ? 1 : 0,
				'picture_name' => $current_picture, // Default ke gambar saat ini
			];
			...
			if (isset($_FILES['picture']) && $_FILES['picture']['error'] == 0) {
				if ($this->upload->do_upload('picture')) {
					...
					if ($current_picture && file_exists('./assets/uploads/products/' . $current_picture)) {
						unlink('./assets/uploads/products/' . $current_picture);
					}

					$product_data['picture_name'] = $new_file_name;
				} else {
					$this->session->set_flashdata('upload_error', $this->upload->display_errors('<div class="text-danger">', '</div>'));
					redirect('admin/products/edit/' . $id); // Kembali ke halaman edit jika upload gagal
				}
			}

			// Update produk utama
			$this->product->edit_product($id, $product_data);
			...
			if (!empty($sub_product_names)) {
				// Hapus semua sub-produk lama
				$this->db->delete('sub_products', ['product_id' => $id]);

				// Tambahkan sub-produk baru
				foreach ($sub_product_names as $key => $name) {
					if (!empty($name) && isset($sub_product_prices[$key]) && is_numeric($sub_product_prices[$key])) {
						$this->db->insert('sub_products', [
							'product_id' => $id,
							'name' => $name,
							'price' => $sub_product_prices[$key],
						]);
					}
				}
			}
